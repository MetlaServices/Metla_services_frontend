...
export const asyncCurrentUser = (): AppThunk => async (dispatch) => {
  try {
    ...
    const response = await axios.post<UserResponse>("/user/currentUser", null, {
      headers: {
...
export const asyncSendClientQuery =
...
export const asyncSendEmployeeQuery =
  (formData: any): AppThunk =>
  async () => {
    try {
...
export const asyncSubmitJobApplication =
  (formData: any): AppThunk =>
  async () => {
    try {
      const config = {
...
export const asyncVerifyOtp =
  (email: string, otp: number, navigate: NavigateFunction): AppThunk =>
  async (dispatch) => {
...
export const asyncGetBlogById =
  (id: string): AppThunk =>
  async (dispatch) => {
    try {
      const response = await axios.get(`/admin/getBlog/${id}`); // Adjust the API endpoint as needed
      await dispatch(saveParticularBlog(response.data.blog));
    } catch (error) {
      console.log(error);
    }
  };

export const asyncGetAllBlogs = (): AppThunk => async (dispatch) => {
  try {
    const response = await axios.get(`/admin/getAllBlogs`); // Adjust the API endpoint as needed
    console.log(response);
    await dispatch(saveBlogs(response.data.blogs));
  } catch (error) {
    console.log(error);
  }
};
