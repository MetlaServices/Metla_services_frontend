...
const Testimonials: React.FC = () => {
  return (
    <div className="testimonial-section p-6 md:p-8 bg-gradient-to-br from-blue-50 to-blue-100 h-auto">
      <h2 className="text-3xl sm:text-4xl font-extrabold text-center mb-8 md:mb-12 text-[#003B5C]">What Our Clients Say</h2>
      
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {testimonials.map((testimonial, index) => (
          <div
            key={index}
            className="testimonial-card bg-white rounded-lg p-6 md:p-8 shadow-lg transition-transform duration-300 hover:shadow-xl hover:scale-105 border-l-4 border-[#005f73]"
          >
            <p className="text-gray-600 mb-4 leading-relaxed italic text-sm md:text-base">{testimonial.feedback}</p>
            <h4 className="text-lg md:text-xl font-semibold text-[#005f73]">{testimonial.name}</h4>
          </div>
        ))}
      </div>
    </div>
